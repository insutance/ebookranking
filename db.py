...
from index.models import Book, TotalBooks   #이거 순서때문에 계속 오류 나던거였냐 .. ?
...
def insert_data():
    ...
    for title,value in kyobo_dic.items():
        Book(title=title, bookstore="kyobo", price=value[1], link=value[2], author=value[3], image=value[4]).save()

    for title,value in yes24_dic.items():
        Book(title=title, bookstore="yes24",price=value[1], link=value[2], author=value[3], image=value[4]).save()

    for title,value in aladin_dic.items():
        Book(title=title, bookstore="aladin",price=value[1], link=value[2], author=value[3], image=value[4]).save()

    for title,value in naver_dic.items():
        Book(title=title, bookstore="naver",price=value[1], link=value[2], author=value[3], image=value[4]).save()

    for title,value in ridibooks_dic.items():
        Book(title=title, bookstore="ridibooks", price=value[1], link=value[2], author=value[3], image=value[4]).save()
